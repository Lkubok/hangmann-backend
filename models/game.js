...
function validateGame(reqNewGame) {
    const schema = {
        player: Joi.string().min(3).max(50).required(),
        level: Joi.string().min(3).max(10).required(),
        lang: Joi.string().min(3).max(15).required(),
    };
...
function validateCheck(reqCheckGame) {
    const schema = {
        ...
        gameId: Joi.string().min(1).max(255).required(),
    };
